...
class Encoder(nn.Module):
    ...
    """
    def __init__(self, number_of_features, hidden_size, hidden_layer_depth, latent_length, dropout, dtype, block = 'LSTM'):
    ...
    def forward(self, x):
        ...
        h_0 = Variable(torch.zeros(
                    self.hidden_layer_depth, x.size(1), self.hidden_size).type(self.dtype), requires_grad = False)

        c_0 = Variable(torch.zeros(
                    self.hidden_layer_depth, x.size(1), self.hidden_size).type(self.dtype), requires_grad = False)

        _, (h_end, c_end) = self.model(x, h_0 if self.block == 'GRU' else (h_0, c_0))
